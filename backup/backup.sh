...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/Floriano21/izinvps/main/ip | grep $MYIP | awk '{print $2}')
...
Save_And_Exit () {
    ...
    git config --global user.name "Floriano21" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/Floriano21/backup
    git push -f https://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@github.com/Floriano21/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/Floriano21/backup/main/$NameUser/$NameUser.zip"
sleep 1
